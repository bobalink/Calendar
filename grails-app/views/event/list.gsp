			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="discription" title="${message(code: 'event.discription.label', default: 'Discription')}" />
					
						<th><g:message code="event.user.label" default="User label" /></th>
				...
				<g:each in="${eventInstanceList}" status="i" var="eventInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: eventInstance, field: "discription")}</td>
					
						<td><g:link action="show" id="${eventInstance.id}">${fieldValue(bean: eventInstance, field: "user.name")}</g:link></td>
